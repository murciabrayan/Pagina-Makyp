...
export function HeroCarousel({ slides, className = '' }: HeroCarouselProps) {
  ...
  useEffect(() => {
    ...
    if (reduced || !autoplay || slides.length < 2) return
  ...
  // Si el equipo dejó el carrusel sin fotos, no se pinta un hueco vacío.
      ...
      <div className="mt-4 flex items-center justify-center gap-2.5">
        {slides.map((bouquet, i) => (
          <button
            ...
            type="button"
            onClick={() => setIndex(i)}
            aria-label={`Ver ramo ${i + 1} de ${slides.length}`}
