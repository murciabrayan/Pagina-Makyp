...
export function Footer({ className = '' }: FooterProps) {
  ...
  // Una red sin enlace no se pinta: antes el enlace de Facebook era '#' y
  // llevaba a ninguna parte, que es peor que no ofrecerlo.
  const socialLinks = [
    site.instagram && { label: 'Instagram', href: site.instagram, icon: <Instagram size={16} /> },
    site.facebook && { label: 'Facebook', href: site.facebook, icon: <Facebook size={16} /> },
    site.tiktok && { label: 'TikTok', href: site.tiktok, icon: <TikTokIcon size={16} /> },
    {
      label: 'WhatsApp',
      href: buildWhatsAppUrl({ phone: site.whatsapp.phone, message: whatsappMessages.general }),
      icon: <WhatsAppIcon size={16} />,
    },
  ].filter((red): red is { label: string; href: string; icon: JSX.Element } => Boolean(red))
